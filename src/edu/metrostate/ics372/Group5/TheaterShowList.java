...
/**
 * This is a collection class for TheaterShow
 *
 */
public class TheaterShowList implements Serializable {
    ...
    /**
     * Searches for show with only show ID
     * 
     * @param ID show's ID
     * @return theaterShow objecti if found
     */
	...
	try {
	    ...
	    for (TheaterShow i : showArray) {
		if (IdServer.contains(i.getcId()) && i.getEnd().compareTo(currentDate) > 0) {
		    output = false;
		}
	    }
