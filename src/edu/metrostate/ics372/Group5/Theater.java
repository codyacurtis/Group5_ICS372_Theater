...
public class Theater implements Serializable {
	...
	if (!PeopleList.personExists(customer)) {
	    PeopleList.insert(customer);
	...
	return (Customer) PeopleList.search(ID);
    }
	...
	Customer customer = PeopleList.customerCreditCard(creditCard);
	return customer;
    }
	...
	PeopleList.listPeople(Customer.class);
    }
	...
	try {
	    ...
	    input.readObject();
	    IdServer.readObject(input);
	    PeopleList.readObject(input);
	...
	try {
	    ...
	    IdServer.writeObject(output);
	    PeopleList.writeObject(output);
	...
	if (!PeopleList.personExists(client)) {
	    PeopleList.insert(client);
	    return client;
	}
	return null;
    }
	...
	Client client;
	client = (Client) PeopleList.search(clientID);
	return client;
    }

    public static void listClient() {
	PeopleList.listPeople(Client.class);
    }
